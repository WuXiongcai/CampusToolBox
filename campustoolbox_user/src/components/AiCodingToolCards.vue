<template>
    <div class="ai-cards">
      ...
      <div v-else-if="aiCodingTools.length === 0" class="empty-state">
        暂无AI编程工具数据
      </div>
        ...
        <div v-for="tool in filteredTools" :key="tool.id" class="resource-card">
          ...
          <div class="card-content">
            ...
            <div class="card-footer">
              <span class="card-category">AI编程</span>
  ...
  import { fetchAiCodingTools, BASE_URL } from '@/api'
  import type { Tool } from '@/types/tool'
  
  const searchQuery = inject('searchQuery', ref(''))
  
  const aiCodingTools = ref<Tool[]>([])
  ...
  const sortedTools = computed(() => {
    return [...aiCodingTools.value].sort((a, b) => {
  ...
  const loadAiCodingTools = async () => {
    loading.value = true
    console.log('开始加载AI编程工具数据...')
    try {
      const res = await fetchAiCodingTools({
        pageNum: currentPage.value,
        pageSize: pageSize.value
      })
      console.log('API响应数据:', res)
      if (res.data.code === "200") {
        aiCodingTools.value = res.data.data.list
        total.value = res.data.data.total
        console.log('成功加载AI编程工具数据:', aiCodingTools.value)
      } else {
        console.warn('加载AI编程工具数据失败:', res.data)
      }
    } catch (error) {
      console.error('加载AI编程工具数据出错:', error)
    } finally {
      loading.value = false
    }
  }
  ...
  onMounted(() => {
    console.log('AiCodingToolCards组件已挂载')
    loadAiCodingTools()
  })
  </script>
