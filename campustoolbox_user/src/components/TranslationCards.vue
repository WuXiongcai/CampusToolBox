<template>
  <div class="ai-cards">
    ...
    <div v-else-if="translationTools.length === 0" class="empty-state">
      暂无在线翻译工具数据
    </div>
...
import { fetchTranslationTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const translationTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...translationTools.value].sort((a, b) => {
...
const loadTranslationTools = async () => {
  loading.value = true
  console.log('开始加载在线翻译工具数据...')
  try {
    const res = await fetchTranslationTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      translationTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载在线翻译工具数据:', translationTools.value)
    } else {
      console.warn('加载在线翻译工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载在线翻译工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('TranslationCards组件已挂载')
  loadTranslationTools()
})
</script>
