<template>
  <div class="ai-cards">
    ...
    <div v-else-if="imageDrawings.length === 0" class="empty-state">
      暂无图像绘画工具数据
    </div>
...
import { fetchImageDrawingData, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const imageDrawings = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...imageDrawings.value].sort((a, b) => {
...
const loadImageDrawings = async () => {
  loading.value = true
  console.log('开始加载图像绘画工具数据...')
  try {
    const res = await fetchImageDrawingData({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      imageDrawings.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载图像绘画工具数据:', imageDrawings.value)
    } else {
      console.warn('加载图像绘画工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载图像绘画工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('ImageDrawingCards组件已挂载')
  loadImageDrawings()
})
</script>
