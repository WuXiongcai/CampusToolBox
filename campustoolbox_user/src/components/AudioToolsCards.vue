<template>
  <div class="ai-cards">
    ...
    <div v-else-if="audioTools.length === 0" class="empty-state">
      暂无AI音频工具数据
    </div>
...
import { fetchAudioTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const audioTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...audioTools.value].sort((a, b) => {
...
const loadAudioTools = async () => {
  loading.value = true
  console.log('开始加载AI音频工具数据...')
  try {
    const res = await fetchAudioTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      audioTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载AI音频工具数据:', audioTools.value)
    } else {
      console.warn('加载AI音频工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载AI音频工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('AudioToolsCards组件已挂载')
  loadAudioTools()
})
</script>
