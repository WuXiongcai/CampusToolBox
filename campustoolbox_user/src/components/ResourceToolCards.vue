<template>
  <div class="resource-cards">
    ...
    <div v-else class="cards-grid">
      <div v-for="resource in filteredTools" :key="resource.id" class="resource-card">
        <div class="card-header">
          <div class="card-icon">
            <img v-if="resource.icon" :src="getIconUrl(resource.icon)" :alt="resource.name" />
        ...
        <div class="card-content">
          <div class="card-description-wrapper">
            <p class="card-description" :class="{ 'expanded': expandedDescriptions[resource.id] }">
              {{ resource.description }}
            </p>
            <button 
              v-if="resource.description.length > 50" 
              class="expand-btn"
              @click="toggleDescription(resource.id)"
            >
              {{ expandedDescriptions[resource.id] ? '收起' : '查看更多' }}
            </button>
          </div>
          <div class="card-footer">
            <span class="card-category">{{ resource.category }}</span>
            <a :href="resource.url" target="_blank" class="card-link">访问链接</a>
          </div>
        </div>
      </div>
    </div>
...
import { fetchResources, BASE_URL } from '@/api'
...
const loadResources = async () => {
  ...
  try {
    ...
    if (res.data.code === "200") {
      ...
      console.log('成功加载资源数据:', resources.value)
    } else {
      console.warn('加载资源数据失败:', res.data)
    }
  } catch (error) {
...
const filteredTools = computed(() => {
  let result = sortedResources.value
...
onMounted(() => {
  console.log('ResourceToolCards组件已挂载')
  loadResources()
})
</script>
