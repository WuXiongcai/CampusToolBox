<template>
  <div class="ai-cards">
    ...
    <div v-else-if="teacherResources.length === 0" class="empty-state">
      暂无教师资源网站数据
    </div>
...
import { fetchTeacherResourcesData, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const teacherResources = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...teacherResources.value].sort((a, b) => {
...
const loadTeacherResources = async () => {
  loading.value = true
  console.log('开始加载教师资源网站数据...')
  try {
    const res = await fetchTeacherResourcesData({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      teacherResources.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载教师资源网站数据:', teacherResources.value)
    } else {
      console.warn('加载教师资源网站数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载教师资源网站数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('TeacherResourcesCards组件已挂载')
  loadTeacherResources()
})
</script>
