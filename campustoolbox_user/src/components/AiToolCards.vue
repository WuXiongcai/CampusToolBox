<template>
  <div class="ai-cards">
    ...
    <div v-else-if="aiTools.length === 0" class="empty-state">
      暂无AI工具数据
    </div>
    <div v-else class="cards-grid">
      <div v-for="tool in filteredTools" :key="tool.id" class="resource-card">
        <div class="card-header">
          <div class="card-icon">
            <img v-if="tool.icon" :src="getIconUrl(tool.icon)" :alt="tool.title" />
        ...
        <div class="card-content">
          ...
          <div class="card-footer">
            <span class="card-category">AI问答</span>
            <a :href="tool.link" target="_blank" class="card-link">访问官网</a>
          </div>
        </div>
      </div>
    </div>
...
import { fetchAiTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const aiTools = ref<Tool[]>([])
...
const loadAiTools = async () => {
  loading.value = true
  console.log('开始加载AI工具数据...')
  try {
    const res = await fetchAiTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      aiTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载AI工具数据:', aiTools.value)
    } else {
      console.warn('加载AI工具数据失败:', res.data)
    }
  } catch (error) {
