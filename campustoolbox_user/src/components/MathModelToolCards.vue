<template>
  <div class="ai-cards">
    ...
    <div v-else-if="mathModelTools.length === 0" class="empty-state">
      暂无数学建模辅助工具数据
    </div>
...
import { fetchMathModelTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const mathModelTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...mathModelTools.value].sort((a, b) => {
...
const loadMathModelTools = async () => {
  loading.value = true
  console.log('开始加载数学建模辅助工具数据...')
  try {
    const res = await fetchMathModelTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      mathModelTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载数学建模辅助工具数据:', mathModelTools.value)
    } else {
      console.warn('加载数学建模辅助工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载数学建模辅助工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('MathModelToolCards组件已挂载')
  loadMathModelTools()
})
</script>
