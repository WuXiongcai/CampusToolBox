<template>
  <div class="ai-cards">
    ...
    <div v-else-if="codeHostingTools.length === 0" class="empty-state">
      暂无代码托管工具数据
    </div>
...
import { fetchCodeHostingTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const codeHostingTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...codeHostingTools.value].sort((a, b) => {
...
const loadCodeHostingTools = async () => {
  loading.value = true
  console.log('开始加载代码托管工具数据...')
  try {
    const res = await fetchCodeHostingTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      codeHostingTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载代码托管工具数据:', codeHostingTools.value)
    } else {
      console.warn('加载代码托管工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载代码托管工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('CodeHostingToolCards组件已挂载')
  loadCodeHostingTools()
})
</script>
