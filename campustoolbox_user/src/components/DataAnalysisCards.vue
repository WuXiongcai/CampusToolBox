<template>
  <div class="ai-cards">
    ...
    <div v-else-if="dataAnalysisTools.length === 0" class="empty-state">
      暂无数据分析工具数据
    </div>
...
import { fetchDataAnalysisData, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const dataAnalysisTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...dataAnalysisTools.value].sort((a, b) => {
...
const loadDataAnalysisTools = async () => {
  loading.value = true
  console.log('开始加载数据分析工具数据...')
  try {
    const res = await fetchDataAnalysisData({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      dataAnalysisTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载数据分析工具数据:', dataAnalysisTools.value)
    } else {
      console.warn('加载数据分析工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载数据分析工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('DataAnalysisCards组件已挂载')
  loadDataAnalysisTools()
})
</script>
