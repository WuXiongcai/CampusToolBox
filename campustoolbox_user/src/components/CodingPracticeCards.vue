<template>
  <div class="ai-cards">
    ...
    <div v-else-if="codingPracticeTools.length === 0" class="empty-state">
      暂无编程刷题网站数据
    </div>
      ...
      <div v-for="tool in filteredTools" :key="tool.id" class="resource-card">
        ...
        <div class="card-content">
          ...
          <div class="card-footer">
            <span class="card-tag" :class="{ 'free': tool.isFree }">
              {{ tool.isFree ? '免费' : '付费' }}
            </span>
...
import { fetchCodingPracticeData, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const codingPracticeTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...codingPracticeTools.value].sort((a, b) => {
...
const loadCodingPracticeTools = async () => {
  loading.value = true
  console.log('开始加载编程刷题网站数据...')
  try {
    const res = await fetchCodingPracticeData({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      codingPracticeTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载编程刷题网站数据:', codingPracticeTools.value)
    } else {
      console.warn('加载编程刷题网站数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载编程刷题网站数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('CodingPracticeCards组件已挂载')
  loadCodingPracticeTools()
})
</script>
