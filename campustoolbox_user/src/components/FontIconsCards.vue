<template>
  <div class="ai-cards">
    ...
    <div v-else-if="fontIcons.length === 0" class="empty-state">
      暂无字体图标工具数据
    </div>
...
import { fetchFontIconsData, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const fontIcons = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...fontIcons.value].sort((a, b) => {
...
const loadFontIcons = async () => {
  loading.value = true
  console.log('开始加载字体图标工具数据...')
  try {
    const res = await fetchFontIconsData({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      fontIcons.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载字体图标工具数据:', fontIcons.value)
    } else {
      console.warn('加载字体图标工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载字体图标工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('FontIconsCards组件已挂载')
  loadFontIcons()
})
</script>
