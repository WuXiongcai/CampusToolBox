<template>
  <div class="ai-cards">
    ...
    <div v-else-if="academicSearchTools.length === 0" class="empty-state">
      暂无学术资源检索网站数据
    </div>
      ...
      <div v-for="tool in filteredTools" :key="tool.id" class="resource-card">
        ...
        <div class="card-content">
          ...
          <div class="card-footer">
            <span class="card-category">学术检索</span>
...
import { fetchAcademicSearchTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const academicSearchTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...academicSearchTools.value].sort((a, b) => {
...
const loadAcademicSearchTools = async () => {
  loading.value = true
  console.log('开始加载学术资源检索网站数据...')
  try {
    const res = await fetchAcademicSearchTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      academicSearchTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载学术资源检索网站数据:', academicSearchTools.value)
    } else {
      console.warn('加载学术资源检索网站数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载学术资源检索网站数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('AcademicSearchCards组件已挂载')
  loadAcademicSearchTools()
})
</script>
