<template>
  <div class="ai-cards">
    ...
    <div v-else-if="aiMindmapTools.length === 0" class="empty-state">
      暂无AI思维导图工具数据
    </div>
...
import { fetchAiMindmapTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const aiMindmapTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...aiMindmapTools.value].sort((a, b) => {
...
const loadAiMindmapTools = async () => {
  loading.value = true
  console.log('开始加载AI思维导图工具数据...')
  try {
    const res = await fetchAiMindmapTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      aiMindmapTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载AI思维导图工具数据:', aiMindmapTools.value)
    } else {
      console.warn('加载AI思维导图工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载AI思维导图工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('AiMindmapToolCards组件已挂载')
  loadAiMindmapTools()
})
</script>
