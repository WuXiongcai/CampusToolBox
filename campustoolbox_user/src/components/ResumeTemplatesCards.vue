<template>
  <div class="ai-cards">
    ...
    <div v-else-if="resumeTemplates.length === 0" class="empty-state">
      暂无简历模板网站数据
    </div>
...
import { fetchResumeTemplatesData, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const resumeTemplates = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...resumeTemplates.value].sort((a, b) => {
...
const loadResumeTemplates = async () => {
  loading.value = true
  console.log('开始加载简历模板网站数据...')
  try {
    const res = await fetchResumeTemplatesData({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      resumeTemplates.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载简历模板网站数据:', resumeTemplates.value)
    } else {
      console.warn('加载简历模板网站数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载简历模板网站数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('ResumeTemplatesCards组件已挂载')
  loadResumeTemplates()
})
</script>
