<template>
  <div class="ai-cards">
    ...
    <div v-else-if="pptTools.length === 0" class="empty-state">
      暂无PPT生成工具数据
    </div>
...
import { fetchPptTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const pptTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...pptTools.value].sort((a, b) => {
...
const loadPptTools = async () => {
  loading.value = true
  console.log('开始加载PPT生成工具数据...')
  try {
    const res = await fetchPptTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      pptTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载PPT生成工具数据:', pptTools.value)
    } else {
      console.warn('加载PPT生成工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载PPT生成工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
