<template>
  <div class="ai-cards">
    ...
    <div v-else-if="chartTools.length === 0" class="empty-state">
      暂无图表工具数据
    </div>
...
import { fetchChartTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const chartTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...chartTools.value].sort((a, b) => {
...
const loadChartTools = async () => {
  loading.value = true
  console.log('开始加载图表工具数据...')
  try {
    const res = await fetchChartTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      chartTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载图表工具数据:', chartTools.value)
    } else {
      console.warn('加载图表工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载图表工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('ChartToolsCards组件已挂载')
  loadChartTools()
})
</script>
