<template>
  <div class="ai-cards">
    ...
    <div v-else-if="fileConvertTools.length === 0" class="empty-state">
      暂无文件格式转换工具数据
    </div>
      ...
      <div v-for="tool in filteredTools" :key="tool.id" class="resource-card">
        ...
        <div class="card-content">
          ...
          <div class="card-footer">
            <span class="card-category">格式转换</span>
...
import { fetchFileConvertTools, BASE_URL } from '@/api'
import type { Tool } from '@/types/tool'

const searchQuery = inject('searchQuery', ref(''))

const fileConvertTools = ref<Tool[]>([])
...
const sortedTools = computed(() => {
  return [...fileConvertTools.value].sort((a, b) => {
...
const loadFileConvertTools = async () => {
  loading.value = true
  console.log('开始加载文件格式转换工具数据...')
  try {
    const res = await fetchFileConvertTools({
      pageNum: currentPage.value,
      pageSize: pageSize.value
    })
    console.log('API响应数据:', res)
    if (res.data.code === "200") {
      fileConvertTools.value = res.data.data.list
      total.value = res.data.data.total
      console.log('成功加载文件格式转换工具数据:', fileConvertTools.value)
    } else {
      console.warn('加载文件格式转换工具数据失败:', res.data)
    }
  } catch (error) {
    console.error('加载文件格式转换工具数据出错:', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  console.log('FileConvertCards组件已挂载')
  loadFileConvertTools()
})
</script>
