...
// 表格相关
let columns = ref([
    { prop: 'title', label: '菜单名称', align: 'left' as const },
    { prop: 'icon', label: '图标' },
    { prop: 'index', label: '路由路径' },
    { prop: 'permiss', label: '权限标识' },
    { prop: 'operator', label: '操作', width: 250 as number }
])
...
const handleEdit = (row: Menus) => {
    rowData.value = { ...row };
    if (row.index && row.index.startsWith('/tools')) {
        if (row.index === '/tools') {
            rowData.value.pid = null;
        } else {
            rowData.value.pid = 14;
        }
    }
    isEdit.value = true;
    visible.value = true;
...
const updateData = () => {
    if (rowData.value.index && rowData.value.index.startsWith('/tools')) {
        ElMessage.warning('工具管理菜单为系统内置，不可修改');
        return;
    }
    closeDialog();
...
const handleDelete = (row: Menus) => {
    if (row.index && row.index.startsWith('/tools')) {
        ElMessage.warning('工具管理菜单为系统内置，不可删除');
        return;
    }
