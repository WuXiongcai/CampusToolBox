<template>
    <div class="container">
        ...
        >
            ...
            <el-table-column prop="link" label="官网链接" show-overflow-tooltip>
                <template #default="scope">
        ...
        >
            ...
            >
                <el-form-item label="图标" prop="icon">
                    ...
                    <div class="upload-tip">点击上传图标，支持JPG、PNG格式</div>
                </el-form-item>
                <el-form-item label="名称" prop="title">
                    <el-input v-model="form.title" placeholder="请输入工具名称" />
                </el-form-item>
                ...
                <el-form-item label="官网链接" prop="link">
                    <el-input v-model="form.link" placeholder="请输入工具官网链接" />
                </el-form-item>
            </el-form>
            <template #footer>
...
const rules = {
    ...
    link: [{ required: true, message: '请输入工具官网链接', trigger: 'blur' }]
}
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await axios.get('/api/ai-tools/list', {
            params: {
                title: query.title,
        ...
        // 直接设置数据，不做额外处理
        if (res.data.code === "200") {  // 注意：这里的code是字符串"200"而不是数字200
...
const handleDelete = async (row) => {
    try {
        ...
        const res = await axios.delete(`/api/ai-tools/${row.id}`)
...
const handleBatchDelete = async () => {
    ...
    try {
        ...
        const res = await axios.delete('/api/ai-tools/batch', {
    ...
    await formRef.value.validate(async (valid) => {
        if (valid) {
            try {
                ...
                const res = await axios[method]('/api/ai-tools', form)
...
// 导出Excel
const handleExport = () => {
    window.open('/api/ai-tools/export', '_blank');
}
</script>
