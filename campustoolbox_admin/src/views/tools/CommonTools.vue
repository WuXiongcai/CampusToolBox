        ...
        >
            ...
            >
                ...
                <el-form-item label="分类" prop="category">
                    <el-input v-model="form.category" placeholder="请输入工具分类" />
                </el-form-item>
...
<script setup lang="ts" name="common-tools">
import { ref, reactive, onMounted} from 'vue';
import { ElMessage, ElMessageBox } from 'element-plus';
import { Check, Picture, Plus } from '@element-plus/icons-vue';
import { fetchCommonToolsData, addCommonTool, updateCommonTool, deleteCommonTool, batchDeleteCommonTools, exportCommonTools } from '@/api';
...
const fetchData = async () => {
    loading.value = true;
    try {
        const res = await fetchCommonToolsData({
            name: query.name,
...
const handleDelete = async (row) => {
    try {
        ...
        const res = await deleteCommonTool(row.id);
...
const handleBatchDelete = async () => {
    ...
    try {
        ...
        const res = await batchDeleteCommonTools(selectedIds.value);
...
// 导出Excel
const handleExport = () => {
    window.open('http://localhost:9090/api/common-tools/export', '_blank');
};
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addCommonTool(form);
                    } else {
                        res = await updateCommonTool(form);
                    }
