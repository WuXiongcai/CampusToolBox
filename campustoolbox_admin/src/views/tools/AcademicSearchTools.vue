...
import { 
    fetchAcademicSearchToolsData,
    addAcademicSearchTool,
    updateAcademicSearchTool,
    deleteAcademicSearchTool,
    batchDeleteAcademicSearchTools,
    exportAcademicSearchTools
} from '@/api'
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchAcademicSearchToolsData(query)
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定要删除该记录吗？', '提示', {
        ...
        try {
            const res = await deleteAcademicSearchTool(row.id)
            if (res.data.code === "200") {
...
const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条记录吗？`, '提示', {
        ...
        try {
            const res = await batchDeleteAcademicSearchTools(selectedIds.value)
            if (res.data.code === "200") {
...
const handleExport = async () => {
    try {
        const res = await exportAcademicSearchTools()
        const blob = new Blob([res.data], { type: 'application/vnd.ms-excel' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = 'academic_search_tools.xlsx'
        link.click()
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addAcademicSearchTool(form)
                    } else {
                        res = await updateAcademicSearchTool(form)
                    }
