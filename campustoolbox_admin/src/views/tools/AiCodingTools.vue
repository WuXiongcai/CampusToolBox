<template>
    <div class="container">
        ...
        >
            ...
            <el-table-column prop="language" label="支持语言" show-overflow-tooltip />
        ...
        >
            ...
            >
                ...
                <el-form-item label="支持语言" prop="language">
                    <el-input v-model="form.language" placeholder="请输入支持的编程语言，多个用逗号分隔" />
                </el-form-item>
            </el-form>
            <template #footer>
...
import { 
    fetchAiCodingToolsData, 
    addAiCodingTool, 
    updateAiCodingTool, 
    deleteAiCodingTool, 
    batchDeleteAiCodingTools,
    exportAiCodingTools
} from '@/api/index'
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchAiCodingToolsData({
            name: query.name,
...
const handleDelete = async (row) => {
    try {
        ...
        const res = await deleteAiCodingTool(row.id)
...
const handleBatchDelete = async () => {
    ...
    try {
        ...
        const res = await batchDeleteAiCodingTools(selectedIds.value)
...
// 导出Excel
const handleExport = () => {
    window.open('http://localhost:9090/api/ai-coding-tools/export', '_blank')
}
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addAiCodingTool(form)
                    } else {
                        res = await updateAiCodingTool(form)
                    }
