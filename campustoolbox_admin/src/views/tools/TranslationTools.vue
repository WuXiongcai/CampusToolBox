...
import {
    fetchTranslationToolsData,
    addTranslationTool,
    updateTranslationTool,
    deleteTranslationTool,
    batchDeleteTranslationTools,
    exportTranslationTools
} from '@/api'
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchTranslationToolsData(query)
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定要删除该记录吗？', '提示', {
        ...
        try {
            const res = await deleteTranslationTool(row.id)
            if (res.data.code === "200") {
...
const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除选中的${selectedIds.value.length}条记录吗？`, '提示', {
        ...
        try {
            const res = await batchDeleteTranslationTools(selectedIds.value)
            if (res.data.code === "200") {
...
const handleExport = async () => {
    try {
        const res = await exportTranslationTools()
        const blob = new Blob([res.data], { type: 'application/vnd.ms-excel' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = 'translation_tools.xlsx'
        link.click()
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addTranslationTool(form)
                    } else {
                        res = await updateTranslationTool(form)
                    }
