...
import {
    fetchResumeTemplatesData,
    addResumeTemplate,
    updateResumeTemplate,
    deleteResumeTemplate,
    batchDeleteResumeTemplates,
    exportResumeTemplates
} from '@/api'
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchResumeTemplatesData(query)
...
const handleExport = async () => {
    try {
        const res = await exportResumeTemplates()
        const blob = new Blob([res.data], { type: 'application/vnd.ms-excel' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = 'resume_templates.xlsx'
        link.click()
...
const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除选中的${selectedIds.value.length}条记录吗？`, '提示', {
        ...
        try {
            const res = await batchDeleteResumeTemplates(selectedIds.value)
            if (res.data.code === "200") {
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addResumeTemplate(form)
                    } else {
                        res = await updateResumeTemplate(form)
                    }
