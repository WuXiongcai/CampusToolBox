...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchAudioToolsData(query)
...
const handleExport = async () => {
    try {
        const res = await exportAudioTools()
        const blob = new Blob([res.data], { type: 'application/vnd.ms-excel' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = 'audio_tools.xlsx'
        link.click()
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定要删除该记录吗？', '提示', {
        ...
        try {
            const res = await deleteAudioTool(row.id)
            if (res.data.code === "200") {
...
const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除选中的${selectedIds.value.length}条记录吗？`, '提示', {
        ...
        try {
            const res = await batchDeleteAudioTools(selectedIds.value)
            if (res.data.code === "200") {
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addAudioTool(form)
                    } else {
                        res = await updateAudioTool(form)
                    }
