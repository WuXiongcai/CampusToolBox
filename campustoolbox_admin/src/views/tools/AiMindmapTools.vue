...
import { 
    fetchAiMindmapToolsData, 
    addAiMindmapTool, 
    updateAiMindmapTool, 
    deleteAiMindmapTool, 
    batchDeleteAiMindmapTools,
    exportAiMindmapTools
} from '@/api/index'
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchAiMindmapToolsData({
            name: query.name,
...
const handleDelete = async (row) => {
    try {
        ...
        const res = await deleteAiMindmapTool(row.id)
...
const handleBatchDelete = async () => {
    ...
    try {
        ...
        const res = await batchDeleteAiMindmapTools(selectedIds.value)
...
// 导出Excel
const handleExport = () => {
    window.open('http://localhost:9090/api/ai-mindmap-tools/export', '_blank')
}
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addAiMindmapTool(form)
                    } else {
                        res = await updateAiMindmapTool(form)
                    }
