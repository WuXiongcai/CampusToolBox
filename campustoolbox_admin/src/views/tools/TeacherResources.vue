...
import {
    fetchTeacherResourcesData,
    addTeacherResource,
    updateTeacherResource,
    deleteTeacherResource,
    batchDeleteTeacherResources,
    exportTeacherResources
} from '@/api'
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await fetchTeacherResourcesData(query)
...
const handleExport = async () => {
    try {
        const res = await exportTeacherResources()
        const blob = new Blob([res.data], { type: 'application/vnd.ms-excel' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = 'teacher_resources.xlsx'
        link.click()
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定要删除该记录吗？', '提示', {
        ...
        try {
            const res = await deleteTeacherResource(row.id)
            if (res.data.code === "200") {
...
const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm(`确定要删除选中的${selectedIds.value.length}条记录吗？`, '提示', {
        ...
        try {
            const res = await batchDeleteTeacherResources(selectedIds.value)
            if (res.data.code === "200") {
            ...
            if (valid) {
                ...
                try {
                    ...
                    if (dialogType.value === 'add') {
                        res = await addTeacherResource(form)
                    } else {
                        res = await updateTeacherResource(form)
                    }
